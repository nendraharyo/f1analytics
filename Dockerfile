...
# and `pip install f1analytics` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "f1analytics", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n f1analytics -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "f1analytics", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "f1analytics", "YOUR_CONSOLE_SCRIPT"]
